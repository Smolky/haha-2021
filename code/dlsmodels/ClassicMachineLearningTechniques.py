...
class ClassicMachineLearningTechniques (BaseModel):
    ...
    all_available_features = ['lf', 'se', 'be', 'ne', 'cf', 'pr']
    ...
    def train (self, force = False, using_official_test = True):
        ...
        # @var classifier_hypermateters Filter only those parameters related to the classifiers we use
        classifier_hypermateters = {key: hyperparameter for key, hyperparameter in hypermateters.items () 
                                    if key.startswith (tuple ([(classifier_key[0] + "__") for classifier_key in classifiers]))}
    

        # @var parameters Dictionary
        parameters = {
            'classifier__selected_model': pipe.named_steps['classifier'].generate (classifier_hypermateters)
        }
