...
class BertModelWithLF (BaseModel):
    ...
    def train (self, using_official_test = True):
        """
        @inherit
        ...
        # @var true_labels List Note that we use the training dataset
        true_labels = sorted (train_df['label'].unique ())
        ...
        for epoch in range (1, self.epochs + 1):
            ...
            cm = sklearn.metrics.confusion_matrix (y_true = y_true, y_pred = y_pred, labels = self.dataset.get_available_labels ())
            confussion_matrix_pretty_printer.print (cm, self.dataset.get_available_labels ())
    ...
    def predict (self, using_official_test = False, callback = None):
        ...
        # run callback
        if callback:
            callback ('bert-lf', y_pred, model_metadata)
